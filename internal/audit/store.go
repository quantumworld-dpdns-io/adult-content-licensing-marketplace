...
func scanEvents(rows *sql.Rows, err error) ([]Event, error) {
	...
	for rows.Next() {
		...
		var e Event
		var metaRaw string
		if err := rows.Scan(&id, &e.Type, &e.ActorSub, &e.ActorRole, &e.EntityID, &metaRaw, &e.At); err != nil { return nil, err }
		_ = json.Unmarshal([]byte(metaRaw), &e.Meta)
		if e.Meta == nil { e.Meta = map[string]any{} }
		out = append(out, e)
	}
...
func (s *SQLStore) Count(ctx context.Context, q Query) (int, error) {
	...
	var n int
	if err := s.db.QueryRowContext(ctx, stmt, args...).Scan(&n); err != nil { return 0, err }
	return n, nil
}
...
func (s *SQLStore) NextCursor(ctx context.Context, q Query) (string, error) {
	...
	var ats []time.Time
	for rows.Next() {
		var id int64; var at time.Time
		if err := rows.Scan(&id, &at); err != nil { return "", err }
		ids = append(ids, id); ats = append(ats, at)
	}
	if err := rows.Err(); err != nil { return "", err }
	if len(ids) < q.Limit { return "", nil }
	idx := len(ids)-1
...
func buildWhere(q Query) ([]string, []any, error) {
	where := []string{"1=1"}
	args := []any{}
	if q.Type != "" { args = append(args, q.Type); where = append(where, "type = $"+strconv.Itoa(len(args))) }
	if q.EntityID != "" { args = append(args, q.EntityID); where = append(where, "entity_id = $"+strconv.Itoa(len(args))) }
	if q.Since != nil { args = append(args, *q.Since); where = append(where, "created_at >= $"+strconv.Itoa(len(args))) }
	if q.Until != nil { args = append(args, *q.Until); where = append(where, "created_at <= $"+strconv.Itoa(len(args))) }
	if q.Cursor != "" {
		c, err := DecodeCursor(q.Cursor)
		if err != nil { return nil, nil, err }
		args = append(args, c.At); aArg := "$" + strconv.Itoa(len(args))
		args = append(args, c.ID); iArg := "$" + strconv.Itoa(len(args))
...
func NewStore(backend string, databaseURL string) (Store, error) {
	if strings.ToLower(strings.TrimSpace(backend)) != "postgres" { return NewMemoryStore(), nil }
	if databaseURL == "" { return nil, fmt.Errorf("DATABASE_URL is required for postgres audit backend") }
	if !driverRegistered("postgres") { return nil, fmt.Errorf("postgres driver not registered for audit store") }
	db, err := sql.Open("postgres", databaseURL)
	if err != nil { return nil, err }
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second); defer cancel()
	if err := db.PingContext(ctx); err != nil { _ = db.Close(); return nil, err }
	if _, err := db.ExecContext(ctx, auditSchemaSQL); err != nil { _ = db.Close(); return nil, err }
	return NewSQLStore(db), nil
}

func driverRegistered(name string) bool {
	for _, d := range sql.Drivers() { if d == name { return true } }
	return false
}
