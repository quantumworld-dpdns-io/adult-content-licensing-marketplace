...
func (r *SQLRepository) Create(ctx context.Context, l License) (License, error) {
	...
	if err := tx.Commit(); err != nil { return License{}, err }
	return l, nil
}
...
func (r *SQLRepository) List(ctx context.Context) ([]License, error) {
	...
	for rows.Next() {
		var l License
		if err := rows.Scan(&l.ID, &l.CreatorID, &l.Title, &l.Description, &l.AITraingProhibited, &l.BasePriceCents, &l.Currency); err != nil { return nil, err }
...
func (r *SQLRepository) Get(ctx context.Context, id string) (License, error) {
	...
	if errors.Is(err, sql.ErrNoRows) { return License{}, ErrNotFound }
	if err != nil { return License{}, err }
	territories, err := r.territoriesForLicense(ctx, l.ID)
	if err != nil { return License{}, err }
	l.TerritoriesISO2Codes = territories
...
func (r *SQLRepository) Update(ctx context.Context, id string, in License) (License, error) {
	...
	if aff == 0 { return License{}, ErrNotFound }

	if _, err := tx.ExecContext(ctx, `DELETE FROM license_territories WHERE license_id=$1`, id); err != nil { return License{}, err }
	for _, iso := range dedupeTerritories(in.TerritoriesISO2Codes) {
		if _, err := tx.ExecContext(ctx, `INSERT INTO license_territories (license_id, iso_code) VALUES ($1,$2)`, id, strings.ToUpper(iso)); err != nil { return License{}, err }
	}
...
func (r *SQLRepository) territoriesForLicense(ctx context.Context, licenseID string) ([]string, error) {
	...
	for rows.Next() {
		var iso string
		if err := rows.Scan(&iso); err != nil { return nil, err }
		out = append(out, iso)
	}
	if err := rows.Err(); err != nil { return nil, err }
	return out, nil
...
func OpenPostgresAndEnsureSchema(ctx context.Context, dsn string) (*sql.DB, error) {
	if dsn == "" { return nil, fmt.Errorf("dsn is required") }
	db, err := sql.Open("postgres", dsn)
	if err != nil { return nil, err }
	if err := db.PingContext(ctx); err != nil { _ = db.Close(); return nil, err }
	if err := EnsureSchema(ctx, db); err != nil { _ = db.Close(); return nil, err }
	return db, nil
}
